...
PKG_SUBDIRS=$(shell go list ./... | grep -v /vendor/ | sed -r 's|github.com/elixirhealth/directory/||g' | sort)
...
acceptance:
	...
	@mkdir -p artifacts
	@go test -tags acceptance -v github.com/elixirhealth/directory/pkg/acceptance 2>&1 | tee artifacts/acceptance.log
...
build-static:
	...
	@CGO_ENABLED=0 ./vendor/$(SERVICE_BASE_PKG)/scripts/build-static deploy/bin/directory
...
docker-image:
	...
	@docker build --rm=false -t gcr.io/elixir-core-prod/directory:snapshot deploy
